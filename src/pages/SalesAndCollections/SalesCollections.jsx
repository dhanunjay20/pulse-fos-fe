...
const SalesCollections = () => {
      ...
      if (selectedProduct) {
        ...
        try {
          ...
          if (invProduct) {
            ...
            row.refillSpace = (invProduct.tankCapacity ?? 0) - (invProduct.currentLevel ?? 0);
  ...
  const totalCollection =
    (parseFloat(cashReceived) || 0) + (parseFloat(phonePay) || 0) + (parseFloat(creditCard) || 0);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!employeeId) return showToast("Please select employee", "error");
  ...
  return (
    <div className="container my-5">
      <h2 className="mb-4 text-primary">Sales & Collections</h2>

      {/* Toast */}
      {toast.show && (
        <div className="toast-container">
          <div
            className={`toast align-items-center text-white bg-${toast.type === "success" ? "success" : "danger"} show animate__animated animate__fadeInRight`}
            role="alert"
          >
            ...
            <div
              className={`toast-progress bg-${toast.type === "success" ? "light" : "light"}`}
            ...
            {[
              ...
              <div className="col-md-6 col-lg-4 mb-3" key={j}>
                ...
                {type === "select" ? (
                  ...
                  <input
                    type="number"
                    className={`form-control ${field === "closing" && p.error ? "is-invalid" : ""}`}
                    value={p[field]}
                    onChange={(e) => handleProductChange(i, field, e.target.value)}
                    readOnly={readOnly}
                  />
